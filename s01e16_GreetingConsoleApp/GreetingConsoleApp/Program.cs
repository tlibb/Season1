...
public class Program
{
    ...
    static Program()
    {
        ...
        if (settings.GreetingWriterClassName == "FileGreetingWriter") // should change this to comparing with InvariantCultureIgnoreCase
        {
        ...
        else if (settings.GreetingWriterClassName == "CsvGreetingWriter")
        {
            _greetingWriter = new CsvGreetingWriter();
        }
        else
        {
            _greetingWriter = new BlackWhiteGreetingWriter(); // this is the default;
        }

        //I don't get the question mark thingy yet!

        //if (settings.GreetingWriterClassName?.Equals("FileGreetingWriter", StringComparison.InvariantCultureIgnoreCase) == true)
        //{ }
    }
    ...
    {
        var myReader = new GreetingReader();
        try
        {
            ProcessGreetings(myReader.ReadGreetingsFromFile("greetings20220118214808.json").ToList());
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Something went wrong: {ex.Message}");
